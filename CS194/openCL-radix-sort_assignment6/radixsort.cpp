...
for (int bitPos = 0; bitPos < 31; bitPos++) {
  ...
  rsort_scan(cv.commands, cv.context,
      ...
      err = clSetKernelArg(kernel_map[reassemble_name_str],0,
			   sizeof(cl_mem), &g_zeros);
      CHK_ERR(err);
      
      err = clSetKernelArg(kernel_map[reassemble_name_str],1,
			   sizeof(cl_mem), &g_ones);
      CHK_ERR(err);

      err = clSetKernelArg(kernel_map[reassemble_name_str],2,
			   sizeof(cl_mem), &g_in);
      CHK_ERR(err);

      err = clSetKernelArg(kernel_map[reassemble_name_str],3,
			   sizeof(cl_mem), &g_out);
      CHK_ERR(err);

      err = clSetKernelArg(kernel_map[reassemble_name_str],4,
			   sizeof(int), &bitPos);
      CHK_ERR(err);
      err = clSetKernelArg(kernel_map[reassemble_name_str],5,
			   sizeof(int), &n);
      CHK_ERR(err);
 ...
 * for radix sort */
void rsort_scan(cl_command_queue &queue, cl_context &context,
		cl_kernel &scan_kern, cl_kernel &update_kern,
