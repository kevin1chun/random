...
int main(int argc, char *argv[])
{
  ...
  CHK_ERR(err);
	...
	/* Copies memory h_Y to the GPU buffer g_Y.
	*/
  ...
  size_t local_work_size[1] = {128};
   ...
   /** These two calls to clSetKernelArg() sets/sends the 
    *  arguments g_Y and n, so that they are available to
    *  the kernel args.
    */
 ...
 /** Here we actually queue/run the incr kernel. */
...
/** presumably shut's down/stops some openCL ready state */
  uninitialize_ocl(cv);
  
  delete [] h_Y;
  delete [] h_YY;

	/** Free the memory we used on the GPU/ */
