...
int main(int argc, char *argv[])
{
  ...
  printf("N = %d worksize (%d, %d) \n", n, (int) local_work_size[0], (int) local_work_size[1]);
  ...
  double t0 = timestamp();
  /* CS194: Launch the matmult kernel
  */
  ...
  err = clEnqueueReadBuffer(cv.commands, g_Y, true, 0, sizeof(float)*n*n,
			    h_Y, 0, NULL, NULL);
  /** Calc the time here, after reading the output buffer, because clEnqueueReadBuffer(x,x, true,...) is blocking
    we are gauranteed to get a sensible value for t0. */
...
t0 = timestamp()-t0;
  ...
  double t1 = timestamp();
  sqr_sgemm(h_YY, h_A, h_B, n); //Calc expected output on CPU
  t1 = timestamp()-t1;
  ...
  for(int i = 0; i < (n*n); i++) {
        ...
        d *= d;
        if(d > 0.0001 ) {
            printf("CPU and GPU results do not match! i = %d, (%d, %d) => Expected: %f Actual: %f \n", i, i / n, i %n, h_YY[i], h_Y[i]);
            break;
        }
    }
  uninitialize_ocl(cv);
